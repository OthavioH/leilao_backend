...
export default class Router {
    ...
    simmetricKey: Buffer<ArrayBufferLike>;

    constructor(server: FastifyInstance, multicastAddress: String, multicastPort: Number, simmetricKey: Buffer<ArrayBufferLike>) {
    ...
    joinLeilaoRoute() {
        this.httpServer.post<{
            Body: { signature: string, user_id: string, message: string };
        }>('/join', async (request, reply) => {
            const { signature, user_id, message } = request.body;

            try {
                var encryptionService = new EncryptionService();
                var isMessageValid = await encryptionService.isMessageValid(message, signature, user_id);
